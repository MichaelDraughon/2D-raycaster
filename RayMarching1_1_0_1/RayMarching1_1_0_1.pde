...
void draw() {
  ...
  for (int i=0; i<viewer.length; i++) {
    for (int j=0; j<40; j++) {
      ...
      } else if (viewer[i].r * cos(viewer[i].t) + viewer[i].x > 1.5 * width || 
        viewer[i].r * cos(viewer[i].t) + viewer[i].x < -0.5 * width || 
        viewer[i].r * sin(viewer[i].t) + viewer[i].y > 1.5 * height || 
        viewer[i].r * sin(viewer[i].t) + viewer[i].y < -0.5 * height) {
      ...
      if (viewer[i].marching) {
        viewer[i].move(mouseX, mouseY);
        viewer[i].march(bounds);
      } else {
        viewer[i].move(mouseX, mouseY);
      }
      strokeWeight(3);
